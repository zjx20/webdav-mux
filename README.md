...
- **上游（upstream）**：内容的来源，二选一：
  - 远程 WebDAV 服务，用 `url` 指定，可选 Basic 认证；
...
| `insecure_skip_verify` | 可选，默认 `false`。上游使用自签名证书时设为 `true`。只对该上游自身的地址生效，跟随重定向到其他地址时仍然校验证书。 |
| `dir` | 本地目录的绝对路径，加载配置时必须存在。 |
...
- 不转发查询参数。
- 上游返回的重定向由本服务自己跟随，最多 10 次，不会回给客户端：
...
| 连不上上游 | `502`；建立连接超时（10 秒）或等待响应头超时（2 分钟）为 `504` |
...
访问日志和错误写到标准错误，格式为 `log/slog` 的文本格式。每个请求一行，包含方法、路径、状态码、字节数、耗时、用户、上游及上游状态码。出错的请求以 `WARN` 级别记录，并附带原因。
